...
class SmartyValidate {
    ...
    function is_registered_validator($id, $form = 'default') {  
        ...
        for($_x = 0, $_y = count($_SESSION['SmartyValidate'][$form]['validators']); $_x < $_y; $_x++) {
            if($_SESSION['SmartyValidate'][$form]['validators'][$_x]['id'] == $id) {
    ...
    function unregister_validator($id, $form = 'default') {  
        if(!SmartyValidate::is_registered_form($form)) {
            trigger_error("SmartyValidate: [unregister_validator] form '$form' is not registered.");
            return false;
        }
        
        for($_x = 0, $_y = count($_SESSION['SmartyValidate'][$form]['validators']); $_x < $_y; $_x++) {
            if($_SESSION['SmartyValidate'][$form]['validators'][$_x]['id'] == $id) {
               unset($_SESSION['SmartyValidate'][$form]['validators'][$_x]);
               break;
            }
        }

 ...
 * @return string
 */
function smarty_function_html_select_date_german($params, &$smarty)
{
    ...
    foreach ($params as $_key=>$_value) {
        switch ($_key) {
            ...
            default:
                $smarty->trigger_error("[html_select_date_german] unknown parameter $_key", E_USER_WARNING);
    ...
    if ($display_months) {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $month_names[$i] = $month_names_de[$i-1];
        ...
        $month_result .= smarty_function_html_options(array('output'     => $month_names,
                                                            ...
                                                            'selected'   => $selected_month ? $selected_month : strftime($month_value_format, mktime(0, 0, 0, (int)$time[1], 1, 2000)),
                                                            'print_result' => false),
                                                      $smarty);
        $month_result .= '</select>';
    }
    ...
    if ($display_days) {
        ...
        $day_result .= smarty_function_html_options(array('output'     => $days,
                                                          'values'     => $day_values,
                                                          'selected'   => $selected_day ? $selected_day : $time[2],
                                                          'print_result' => false),
                                                    $smarty);
        $day_result .= '</select>';
    }
    ...
    if ($display_years) {
        ...
        if ($year_as_text) {
            ...
            $year_result .= smarty_function_html_options(array('output' => $years,
                                                               'values' => $yearvals,
                                                               'selected'   => $selected_year ? $selected_year : $time[0],
                                                               'print_result' => false),
                                                         $smarty);
            $year_result .= '</select>';
        }
